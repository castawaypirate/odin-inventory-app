...
- fix dynamic url building, more side bar filters, fix fors and ids ~ 16/9/2026 ~
...
- ~ 15/9/2026 ~:
    - learn how to dynamically construct the url with the url params taking into consideration params of two or more forms
    - create query to match the exact genres that are passed through the params with group by and having count
    - labels' fors should match with inputs' ids
    - dynamic string built query along with the params array
    - onchange="this.form.submit()" bypasses the event listener on submit
- ~ 14/9/2026 ~:
